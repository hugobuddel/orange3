...
class OWScatterPlotGraphQt_test(OWWidget, ScaleScatterPlotData):
    ...
    def updateData(self, xAttr, yAttr, colorAttr, shapeAttr = "", sizeShapeAttr = "", labelAttr = None, **args):
        #print("OWScatterPlotGraphQt_test updateData")
        # self.legend().clear()
        self.tooltipData = []
        ...
        #self.pgScatterPlotItem.scene().sigWheelEvent.connect(self.wheelEvent)    # scene() is changed only when you add scatterplotitem on plotwidget
        #self.spi.scene().sigWheelEvent.connect(self.wheelEvent)    # scene() is changed only when you add scatterplotitem on plotwidget
